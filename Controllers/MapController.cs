...
namespace InsuranceWebApp.Controllers
{
    public class MapController : Controller
    {
        ...
        public async Task<IActionResult> Index(double latitude = 106.7035, double longitude = 106.7035, int range = 5)
        {
            var hospitalViewModels = await _mapService.GetNearByHospitalAsync(new GeoPoint(latitude, longitude), range);
            if (Request.Headers.ContentType == HttpContentTypeFormat.JSON)
            {
                return Json(hospitalViewModels);
            }
