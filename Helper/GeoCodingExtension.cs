...
namespace InsuranceWebApp.Helper
{
    ...
    {
        public static List<HospitalViewModel> FindNearByLocation(this GeoPoint currentPosition, List<HospitalViewModel> locations, int range)
        {
            locations.Sort(new DistanceComparator(currentPosition));// tu gan nhat den xa nhat
            if (locations.Count <= range)
                return locations;
            return locations.Take(range)
                    .ToList();
        }
